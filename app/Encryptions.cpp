...
string Encryption::encryption(QString Pass, QString Text, string cipher, bool encrypt)
{
    ...
    if (cipher == "AES")
    {
        ...
        StringSource( reinterpret_cast<const char*>(pass.c_str()), true, new HashFilter(*(new SHA256), new ArraySink(key, AES::MAX_KEYLENGTH )) );
        memset( iv, 0x00, AES::BLOCKSIZE );
        if (encrypt)
        {
            CBC_Mode<AES>::Encryption Encryptor( key, sizeof(key), iv );
            StringSource( text, true, new StreamTransformationFilter( Encryptor,new HexEncoder(new StringSink( CipherText ),false ) ) );
        }
        else
        {
            try
    ...
    if (cipher == "AES_base64")
    {
       byte key[ AES::MAX_KEYLENGTH], iv[ AES::BLOCKSIZE ];
        ...
        {
            CBC_Mode<AES>::Encryption Encryptor( key, sizeof(key), iv );
            StringSource( text, true, new StreamTransformationFilter( Encryptor,new Base64Encoder(new StringSink( CipherText ),false ) ) );
        }
        else
        {
            try
            {
                CBC_Mode<AES>::Decryption Decryptor( key, sizeof(key), iv );
                StringSource( text, true, new Base64Decoder(new StreamTransformationFilter( Decryptor, new StringSink( RecoveredText ) ) ) );
            }
            catch ( Exception& e)
            {
                return e.what();
